    ...
    */
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		*/
		...
		$query = $this->aauth_db->get($this->config_vars['voters']);
		$row = $query->row();
	 ...
	 * Sets a voters 'last login attempts' to null
	...
	public	function reset_login_attempts($user_id) {
		...
		return $this->aauth_db->update($this->config_vars['voters'], $data);
	}
	...
	public function remind_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$this->aauth_db->update($this->config_vars['voters'], $data);
			mail($email, "Reset Password", $this->CI->lang->line('aauth_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $row->id . '/' . $ver_code );
	...
	public function reset_password($user_id, $ver_code){
		...
		if( $query->num_rows() > 0 ){
			...
			$this->aauth_db->where('id', $user_id);
			$this->aauth_db->update($this->config_vars['voters'] , $data);
			mail($email, $this->CI->lang->line('aauth_email_reset_success_subject'), $this->CI->lang->line('aauth_email_reset_success_new_password') . $pass);
/*
			$this->CI->email->from( $this->config_vars['email'], $this->config_vars['name']);
...
*/			return TRUE;
	 ...
	 */
	public function create_user($surname, $firstname, $othername, $dateofbirth, $gender, $phone, $occupation,$picture, /* $email,*/ $pass, $name) {
		...
		if ( $this->aauth_db->insert($this->config_vars['voters'], $data )){

			$user_id = $this->aauth_db->insert_id();

			// set default group
			$this->add_member($user_id, $this->config_vars['voter_group']);
	...
	/**
	 * List voters
	 * Return voters as an object array
	 * @param bool|int $group_par Specify group id to list group or FALSE for all voters
	 * @param string $limit Limit of voters to be returned
	 * @param bool $offset Offset for limited number of voters
	 * @param bool $include_banneds Include banned voters
	 * @return array Array of voters
	 */
	public function list_voters($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
		...
		if ($group_par != FALSE) {
			...
			$this->aauth_db->select('*')
				->from($this->config_vars['voters'])
				->join($this->config_vars['user_to_group'], $this->config_vars['voters'] . ".id = " . $this->config_vars['user_to_group'] . ".user_id")
				->where($this->config_vars['user_to_group'] . ".group_id", $group_par);

			// if group_par is not given, lists all voters
		} else {
	...
	public function delete_user($user_id) {
		...
		// delete user
		$this->aauth_db->where('id', $user_id);
		return $this->aauth_db->delete($this->config_vars['voters']);
	...
	public function delete_group($group_par) {
		...
		// bug fixed
		// now voters are deleted from user_to_group table
	...
	/**
	 * List Banned voters
	 * List all Banned voters
	 * @return object Array of Banned voters
	 */
 ...
 * list_voters da grup_par verilirse ve adamın birden fazla grubu varsa nolurkun? // bi denemek lazım belki distinct ile düzelir
